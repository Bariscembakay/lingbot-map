...
TAP23 = 3          # index of layer 23 within the 4 cached taps
HALF = 1024
...
def query_frame(rng: np.random.Generator, step: int, window: int,
                min_gap: int, in_window_prob: float = 0.5) -> Optional[int]:
    """Pick a past frame to query for Loss 2.

    Half the samples land inside the BPTT window, where gradient can reach the
    write that stored them; half land further back, where the only lesson
    available is "do not destroy what is already there". `min_gap` keeps the task
    from degenerating into copying a write that just happened.
    """
    hi = step - min_gap
    if hi < 0:
        return None
    if rng.random() < in_window_prob:
        lo = max(0, step - window)
        return int(rng.integers(lo, hi + 1)) if hi >= lo else None
    hi2 = min(hi, step - window - 1)
    return int(rng.integers(0, hi2 + 1)) if hi2 >= 0 else None
