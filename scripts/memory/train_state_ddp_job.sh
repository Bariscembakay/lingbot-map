#!/usr/bin/env bash
# One MULTI-GPU training run of the CUT3R-state recall model (torchrun, one node).
# Usage: train_state_ddp_job.sh <out_dir> <clip_glob_or_dir> [extra train_state.py args]
# GPU count is taken from the allocation (SLURM_GPUS_ON_NODE).
set -euo pipefail
OUT="$1"; shift
CLIPS_SPEC="$1"; shift
...
# KEEPALIVE=1: reaper guard (covers every visible GPU). The big run's CPU-side
# clip load (~450 GB from /group into host RAM) leaves the GPUs idle long
# enough to look reaper-worthy.
