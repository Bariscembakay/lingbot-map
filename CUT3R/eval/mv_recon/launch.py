...
def get_args_parser():
    ...
    # LOCAL PATCH (lingbot-map): select which of datasets_all to run.
    # Selection only -- the model, protocol and metric code below are untouched.
    parser.add_argument(
        "--datasets", type=str, default="", 
        help="comma-separated subset of datasets_all; empty = all"
    )
    return parser
...
def main(args):
    ...
    if args.datasets:
        ...
        missing = [d for d in keep if d not in datasets_all]
        assert not missing, f"unknown dataset(s): {missing}; have {list(datasets_all)}"
        datasets_all = {k: v for k, v in datasets_all.items() if k in keep}
        print(f"[local patch] running only: {list(datasets_all)}")
