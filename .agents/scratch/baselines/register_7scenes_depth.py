...
src_folder = '/group/compact-3dmem/datasets/7-scenes'
focal_length = 525.0
	...
	if True:
		...
		if True:
			...
			for d_index, d_file in enumerate(depth_files):
				if d_index % 1000 == 0: 
					print(d_index, ds, seq)
				out_path = ds + '/' + seq + '/' + d_file.replace("depth.png", "depth.proj.png")
...
ALL = []
for scan in ['chess', 'fire', 'heads', 'office', 'pumpkin', 'redkitchen', 'stairs']:
	d = os.path.join(src_folder, scan)
	for seq in sorted(os.listdir(d)):
		if seq.startswith('seq-') and os.path.isdir(os.path.join(d, seq)):
			ALL.append((d, seq))

Parallel(n_jobs=16, verbose=5)(
	delayed(process_seq)(d, seq) for d, seq in ALL)
